...
static void init_cpu_state(agc_t *State)
{
    ...
    // PRIORITY slot pre-mark: keep this even though upstream doesn't,
    // because our prior cold-boot deadlock was traceable to slot 0
    // being "occupied at priority 0" before FRESH-START's LXCH PRIORITY
    // block could mark it empty. See agc_harness/test_executive_state
    // for the trace. Removing this regresses to the original CHARIN
    // dispatch failure. Upstream gets away with leaving slots at 0
    // because its FRESH-START completes before our integration's first
    // ChannelRoutine fires; ours doesn't.
