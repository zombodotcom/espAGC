...
// Canonical SocketAPI port. Every engine-cycle I/O call forwards into
// yaagc_socket — peripherals talk to us over TCP using the same 4-byte
// protocol as canonical yaAGC, and local touch/web/serial keypresses
// enter the same drain via yaagc_socket_inject_key. channel_router
// stays in the output chain so the LCD keeps mirroring.
...
void ChannelOutput(agc_t *State, int Channel, int Value)
{
    ...
    // channel_router always sees outputs so the local LCD keeps tracking
    // the engine even without a connected TCP peer.
...
#endif
    // Canonical path: TCP peers and local injectors (touch/web/serial
    // via channel_router_post_key → yaagc_socket_inject_key) share the
    // same synthetic-client byte ring. yaagc_socket_channel_input
    // drains it and does WriteIO + IR5 from inside ChannelInput, which
    // is the only entry point agc_engine calls per cycle.
