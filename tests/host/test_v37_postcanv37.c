// test_v37_postcanv37 — log the FIRST ~200 distinct Z addresses
// reached after the engine enters CANV37 (04,02134). This shows the
// interpretive CALL INTSTALL path.
...
int main(void)
{
    ...
    harness_post_key(16);   // 0
    for (long i = 0; i < 50000; i++) agc_engine(s);
    harness_post_key(28);   // E (final)

    // Now log every distinct (bank, Z) until we hit MODREG-set or 500 steps.
    ...
    for (long long i = 0; i < 50000000LL; i++) {
        ...
        if (b == 04 && z == 02223) {
            printf("  *** MODREG set! cyc=%lld\n", cyc);
            found_modreg = 1;
            break;
        }
        if (found_v37 && captured < 300 && (b != last_bank || z != last_z)) {
            printf("  step%4d cyc=%lld %02o,%05o\n",
                   captured, cyc, b, z);
            captured++;
            last_bank = b;
            last_z = z;
    ...
    if (!found_modreg) printf("  *** MODREG never set after %lld cycles\n", cyc);
    printf("  end MODREG=%05o\n", s->Erasable[2][011] & 077777);
